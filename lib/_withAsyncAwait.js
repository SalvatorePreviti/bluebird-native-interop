...
const staticMembers = {
  ...
  async each(promises, iterator) {
    if (typeof promises === 'object' && promises !== null && typeof promises.then === 'function') {
      promises = await promises
    }
    const all = await this.all(promises)
    const length = all.length
    for (let index = 0; index !== length; index++) {
      await iterator(all[index], index, length)
    }
    return all
  },

  async mapSeries(promises, iterator) {
    if (typeof promises === 'object' && promises !== null && typeof promises.then === 'function') {
      promises = await promises
    }
    const all = await this.all(promises)
    const length = all.length
    for (let index = 0; index !== length; index++) {
      all[index] = await iterator(all[index], index, length)
    }
    return all
  },
...
const prototypeMembers = {
  ...
  async each(iterator) {
    const all = await this.constructor.all(await this)
    const length = all.length
    for (let index = 0; index !== length; index++) {
      await iterator(all[index], index, length)
    }
  ...
  async mapSeries(iterator) {
    const all = await this.all(await this)
    const length = all.length
    for (let index = 0; index !== length; index++) {
      all[index] = await iterator(all[index], index, length)
    }
